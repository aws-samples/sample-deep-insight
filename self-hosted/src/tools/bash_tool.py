...
def handle_bash_tool(cmd: Annotated[str, "The bash command to be executed."]):
    ...
    Security Note: This function uses shell=True for subprocess execution.
    This is intentional as the tool is designed to execute arbitrary bash commands
    from trusted AI agents in a controlled environment. Input commands are generated
    by the agent system, not from untrusted external sources.
    """
    ...
    try:
        ...
        # SECURITY: shell=True is intentional for bash command execution
        # Commands are generated by trusted AI agents, not external user input
